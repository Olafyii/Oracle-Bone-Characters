...
txt = open(os.path.join(root_path, 'images_labels.txt'))
lines = txt.readlines()
txt.close()
...
train = open(os.path.join(root_path, 'images_labels_train.txt'), 'w')
for i, xi in enumerate(x_train):
    train.write(xi+' '+y_train[i]+'\n')
train.close()
test = open(os.path.join(root_path, 'images_labels_test.txt'), 'w')
